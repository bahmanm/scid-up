...
proc ::enginelist::choose {} {
    ...
    if {[winfo exists $w]} {
        ...
        return }
    win::createDialog $w
    ::setTitle $w "[tr ScidUp]: [tr ToolsAnalysis]"
    ttk::frame $w.buttons
    ttk::frame $w.list
    # Set up enginelist
...
proc ::enginelist::delete {index} {
    ...
    set answer [tk_messageBox -title [tr ScidUp] -icon question -type yesno \
            -message $msg]
    if {$answer == "yes"} {
...
proc ::enginelist::edit {index} {
            ...
            ttk::button $f.b$i -text "..." -command {
                if {$::windowsOS} {
                    ...
                    set fName [tk_getOpenFile -initialdir $engines(newDir) \
                        -title "[tr ScidUp]: [tr ToolsAnalysis]" -filetypes $ftype]
                } else {
                    set fName [tk_getOpenFile -initialdir $engines(newDir) \
                        -title "[tr ScidUp]: [tr ToolsAnalysis]"]
                }
                if {$fName != ""} {
    ...
    ttk::button $f.ok -text OK -command {
        if {[string trim $engines(newName)] == ""  ||
            ...
            tk_messageBox -title [tr ScidUp] -icon info \
...
proc configAnnotation {} {
    ...
    win::createDialog $w
    ::setTitle $w "[tr ScidUp]: $::tr(Annotate)"
    wm resizable $w 0 0
    set f [ttk::frame $w.f]
    pack $f -expand 1
...
proc makeAnalysisWin { {n 1} {index -1} {autostart 1}} {
    ...
    if {$open_err} {
        tk_messageBox -title "[tr ScidUp]: error starting analysis" \
    ...
    if {$analysis(logMax) > 0} {
        if {! [catch {open [file join $::scidLogDir "engine$n.log"] w} log]} {
            ...
            logEngine $n "ScidUp-Engine communication log file"
            logEngine $n "Engine: $analysisName"
            logEngine $n "Command: $analysisCommand"
            logEngine $n "Date: [clock format [clock seconds]]"
            logEngine $n ""
            logEngine $n "This file was automatically generated by ScidUp."
            logEngine $n "It is rewritten every time an engine is started in ScidUp."
            logEngine $n ""
        }
    }
...
proc toggleFinishGame { { n 1 } } {
	...
	win::createDialog $w
	wm resizable $w 0 0
	::setTitle $w "[tr ScidUp]: $::tr(FinishGame)"
...
proc setAutomoveTime {{n 1}} {
    ...
    ::setTitle $w "[tr ScidUp]: Engine thinking time"
    wm resizable $w 0 0
    ttk::frame $w.f
    pack $w.f -expand 1
